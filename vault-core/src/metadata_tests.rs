...
mod tests {
    ...
    fn test_metadata_sections_basic_operations() {
        ...
        vault.set_metadata_section(MetadataSectionType::UserSettings, test_data.clone()).unwrap();

        // Test getting metadata section
        let retrieved_data = vault.get_metadata_section(&MetadataSectionType::UserSettings).unwrap();
        assert_eq!(retrieved_data, Some(test_data.clone()));

        // Test getting non-existent section
        let non_existent = vault.get_metadata_section(&MetadataSectionType::AuditLog).unwrap();
        assert_eq!(non_existent, None);

        // Test removing metadata section
        let removed = vault.remove_metadata_section(&MetadataSectionType::UserSettings).unwrap();
        assert!(removed);

        // Verify it's gone
        let after_removal = vault.get_metadata_section(&MetadataSectionType::UserSettings).unwrap();
        assert_eq!(after_removal, None);

        // Test removing non-existent section
        let not_removed = vault.remove_metadata_section(&MetadataSectionType::UserSettings).unwrap();
        assert!(!not_removed);
    }
    ...
    fn test_metadata_sections_persistence() {
        ...
        // Create vault and add metadata
        {
            ...
            vault.set_metadata_section(MetadataSectionType::UserSettings, user_settings.clone()).unwrap();
            vault.set_metadata_section(MetadataSectionType::AuditLog, audit_log.clone()).unwrap();
            
            // Verify data is there
            assert_eq!(vault.get_metadata_section(&MetadataSectionType::UserSettings).unwrap(), Some(user_settings));
            assert_eq!(vault.get_metadata_section(&MetadataSectionType::AuditLog).unwrap(), Some(audit_log));
        }
        ...
        // Reopen vault and verify persistence
        {
            ...
            let user_settings = vault.get_metadata_section(&MetadataSectionType::UserSettings).unwrap();
            let audit_log = vault.get_metadata_section(&MetadataSectionType::AuditLog).unwrap();
    ...
    fn test_metadata_sections_update_existing() {
        ...
        vault.set_metadata_section(MetadataSectionType::UserSettings, initial_data).unwrap();

        // Update with new data
        let updated_data = b"updated data".to_vec();
        vault.set_metadata_section(MetadataSectionType::UserSettings, updated_data.clone()).unwrap();

        // Verify updated data
        let retrieved = vault.get_metadata_section(&MetadataSectionType::UserSettings).unwrap();
    ...
    fn test_metadata_sections_multiple_types() {
        ...
        vault.set_metadata_section(MetadataSectionType::UserSettings, user_settings.clone()).unwrap();
        vault.set_metadata_section(MetadataSectionType::SharingKeys, sharing_keys.clone()).unwrap();
        vault.set_metadata_section(MetadataSectionType::RecoveryInfo, recovery_info.clone()).unwrap();

        // Verify all are stored correctly
        assert_eq!(vault.get_metadata_section(&MetadataSectionType::UserSettings).unwrap(), Some(user_settings));
        assert_eq!(vault.get_metadata_section(&MetadataSectionType::SharingKeys).unwrap(), Some(sharing_keys));
        assert_eq!(vault.get_metadata_section(&MetadataSectionType::RecoveryInfo).unwrap(), Some(recovery_info));

        // Remove one and verify others remain
        vault.remove_metadata_section(&MetadataSectionType::SharingKeys).unwrap();
        
        assert_eq!(vault.get_metadata_section(&MetadataSectionType::UserSettings).unwrap(), Some(b"user settings".to_vec()));
        assert_eq!(vault.get_metadata_section(&MetadataSectionType::SharingKeys).unwrap(), None);
        assert_eq!(vault.get_metadata_section(&MetadataSectionType::RecoveryInfo).unwrap(), Some(b"recovery information".to_vec()));
    }
    ...
    fn test_metadata_sections_empty_data() {
        ...
        vault.set_metadata_section(MetadataSectionType::UserSettings, empty_data.clone()).unwrap();
    ...
    fn test_metadata_sections_large_data() {
        ...
        vault.set_metadata_section(MetadataSectionType::AuditLog, large_data.clone()).unwrap();

        // Verify large data is stored and retrieved correctly
        let retrieved = vault.get_metadata_section(&MetadataSectionType::AuditLog).unwrap();

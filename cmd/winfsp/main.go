...
func main() {
	...
	logger.Info("Starting dirLocker WinFSP daemon",
		"vault", *vaultPath,
		"drive", *driveLetter,
		"readonly", *readOnly)
	...
	// Set up WinFSP filesystem
	fs, err := winfsp.NewVaultFS(v, logger, &winfsp.Options{
		ReadOnly: *readOnly,
		Debug:    *debug,
	})
	if err != nil {
		logger.Error("Failed to create WinFSP filesystem", "error", err)
		os.Exit(1)
	}
	...
	// Mount and serve
	logger.Info("Mounting WinFSP filesystem", "drive", *driveLetter)
	if err := fs.Mount(ctx, *driveLetter); err != nil {
		logger.Error("Failed to mount filesystem", "error", err)
		os.Exit(1)
	}

	logger.Info("WinFSP filesystem mounted successfully")
	...
	logger.Info("dirLocker WinFSP daemon stopped")
}
